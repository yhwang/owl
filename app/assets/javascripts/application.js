    ...
    if (typeof google == 'undefined') {
      $.loadScript('https://maps.googleapis.com/maps/api/js?key=AIzaSyAwpA8PHX57_8RCU8iCCDdIEViCWrpy44k', function() {
        ...
        $.get('/getincidentlocation', function(loc) {
          ...
          // Geocode origin & dest
          let geocoder; geocoder = new google.maps.Geocoder(); geocoder.geocode({'address': 'talahassee, florida'}, function(results, status) {
            if (status == 'OK') {
...
function showMapping() {
  ...
  setTimeout(function() {
    ...
    const mapsHtml = '<div id="maps"><div id="navigation"><div class="navlist"><div id="directionsPanel" style="height 100%;"></div></div></div><div id="mapOverlay"><h3>Loading maps...<span></span></h3></div><div id="map"></div></div>';
    $('#eye').append(mapsHtml);
...
function getForecast(geocode) {
  ...
  $.get('/getweatherforecast?geoid=' + g, function(data) {
    ...
    const jResponse = JSON.parse($('.data-response').html());

    // // unpack the json
    console.log(jResponse);

    const daypart = jResponse.daypart;
    const dayofweek = jResponse.dayOfWeek;
    const narrative = jResponse.narrative;
    const tempmin = jResponse.temperatureMin;
    const tempmax = jResponse.temperatureMax;
    ...
    $('.four-day-forecast .day-container').each(function(index, el) {
      ...
      // update icons
      const s = narrative[index].toLowerCase();
...
function getNowcast(geocode) {
  ...
  $.get('/weathernowcast?location=' + g, function(data) {
    ...
    const desc = jResponse.forecast.narrative_256char;
...
function getTropicalCurrentPosition() {
  $.get('/tropicalcurrent', function(data) {
    ...
    const advisoryinfo = jResponse.advisoryinfo;
    const laststorm = advisoryinfo[0];
    const stormname = laststorm.storm_name;
    const stormtype = laststorm.currentposition.storm_type;
    const stormcategory = laststorm.currentposition.storm_sub_type;
...
function checknotifications() {
  $.get('/checknotifications', function(data) {
    ...
    const n = $('.notifications-list .notification').first();
    const nid = n.attr('id');
    console.log(nid);
    console.log(data);
...
function checkpriorities() {
  $.get('/checkpriorities', function(data) {
    // get id of last notification and check if it matches last notification in panel
    const n = $('.priorities-list .priority').first();
    const nid = n.attr('id');
    console.log(nid);
    console.log(data);
...
function getnotification() {
  $.get('/getnotification', function(data) {
    ...
    // add notification
    const nhtml = '<div class="notification active out" id="' + nid + '">' + tw + '<div class="info"><h4>' + t + '</h4><p>' + c + '</p></div><div class="time"><p><span>1</span>mins</p></div></div>';
...
function getpriority() {
  $.get('/getpriority', function(data) {
    ...
    const num = $('.priority').length + 1;

    // add notification

    const phtml = '<div class="priority out" id="' + nid + '"><h4>' + t + '</h4><p>' + c + '</p><div class="num">' + num + '</div></div>';
...
function getLocation(g) {
  $.get('/getlocation?location=' + g, function(data) {
    ...
    // grab the raw response and turn into json
    const jResponse = JSON.parse(raw.html());
    console.log(jResponse);

    // coordinates
    const lat = jResponse.location.latitude[0];
    const lon = jResponse.location.longitude[0];
    console.log(lat);
    console.log(lon);
  ...
  for (var i = 25 - 1; i >= 0; i--) {
    ...
    marker.addListener('click', function() {
      ...
      setTimeout(function() {
        ...
        $('.console p').remove();
        console.log(e[0]);
        const l = e[0].label.text;
        const lat = e[0].position.lat();
        const lng = e[0].position.lng();
...
function loadNewIncidentMap(newloc) {
  const mapsHtml = '<div id="maps"><div id="mapOverlay"></div><div id="map"></div></div>';
  ...
  // Geocode origin & dest
  let geocoder; geocoder = new google.maps.Geocoder(); geocoder.geocode({'address': newloc}, function(results, status) {
    if (status == 'OK') {
...
function loadCivilianDestination(destination, scenic) {
  ...
  const origin = '410 south 4th street brooklyn new york';
  let oLat = 0;
  let oLng = 0;

  // Geocode origin & dest
  let geocoder; geocoder = new google.maps.Geocoder(); geocoder.geocode( {'address': origin}, function(results, status) {
    if (status == 'OK') {
      oLat = results[0].geometry.location.lat(); oLng = results[0].geometry.location.lng();
    } else {
      console.log('GeoCode failed: ' + status);
    }
  });
  geocoder = new google.maps.Geocoder(); geocoder.geocode( {'address': mapQuery}, function(results, status) {
    if (status == 'OK') {
      newLat = results[0].geometry.location.lat(); newLng = results[0].geometry.location.lng();
    } else {
      console.log('GeoCode failed: ' + status);
    }
  });
...
function getGeocode(address, lat, lng) {
  let geocoder; geocoder = new google.maps.Geocoder(); geocoder.geocode( {'address': address}, function(results, status) {
    if (status == 'OK') {
  ...
  directionsService.route(request, function(response, status) {
    ...
    if (status == 'OK') {
      ...
      setTimeout(function() {
        ...
        const minInt = routeTimeEst.replace(/[^0-9.]/g, '');
        const est = parseInt(m) + parseInt(minInt);
        const totalEst = ((parseInt(h) + 11) % 12 + 1) + ':' + ((minInt + 59) % 60 + 1);
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      data.results.forEach(function(el, index) {
        const fullName = el.name.first + ' ' + el.name.last;
        const fullUsername = el.name.first[0] + el.name.last;
...
function identifyLanguage(text) {
  $.get('/identifylanguage?text=' + text, function(data) {
    const d = data;
    const j = JSON.parse(d).languages[0].language;
...
function embedWaves() {
  const embed = '<canvas id="waves"></canvas>';
  ...
  if (intents.indexOf('translate') > -1) {
    ...
    if (entities.indexOf('french') > -1) {
      ...
      $.get('/translate?startlang=en&targetlang=fr&text=' + text, function(data) {
        const r = JSON.parse(data).translations[0].translation;
        console.log(r);
        machineResponse = r;
        writeDialogue(machineResponse, 'machine');
...
function writeDialogue(m, t, d) {
  ...
  setTimeout(function() {
    ...
    // scroll element
    const el = $('.dialogue .message').last();
