...
class MainController < ApplicationController
  ...
  def search
    ...
    User.all.order(username: :asc).each_with_index do |f, _index|
      @el = {
        value: f.username,
  ...
  def getdirectmessages
    ...
    @u2 = params[:recipient]
    @messages = []
    Message.where('sender = ? AND recipient = ?', @u1.to_i, @u2.to_i)
           .order(created_at: :desc).each do |f|
      @m = [f.sender, f.message, f.created_at.to_i]
      @messages.push(@m)
    end
    Message.where('sender = ? AND recipient = ?', @u2.to_i, @u1.to_i)
           .order(created_at: :desc).each do |f|
      @m = [f.sender, f.message, f.created_at.to_i]
      @messages.push(@m)
    end
    @messages = @messages.sort_by { |a| a[2] }
    respond_to do |format|
  ...
  def setmessageread
    ...
    # set all messages in these two users' thread to "read=true" because maybe
    # several were unread
  ...
  def createuser
    ...
    @u = User.create(username: @username, name: @name, password: 'password',
  ...
  def getnotification
    ...
    respond_to do |format|
      format.json do
        render json: { title: @ns.title, content: @ns.content, nid: @ns.id }
      end
    end
  end
  ...
  def getpriority
    ...
    respond_to do |format|
      format.json do
        render json: { title: @ns.name, content: @ns.details, nid: @ns.id }
      end
    end
  end
  ...
  def sendtext
    ...
    @message = if params[:message].blank?
                 'Hi there, this is OWL.'
               else
                 params[:message]
               end
  ...
  def analyzetone
    ...
    uri = URI.parse(
      'https://gateway.watsonplatform.net/tone-analyzer/api/v3/tone?text=' +
      @text + '&version=2017-09-21&sentences=true'
    )
  ...
  def visualizer
    ...
    # @endpoint = "https://gateway.watsonplatform.net/visual-recognition/"\
    #             "api/v3/classify?version=2016-05-17&classifier_ids="\
    #             "DefaultCustomModel_2101976&url=" + @imgurl
    @endpoint = 'https://gateway.watsonplatform.net/visual-recognition/api/v3/'\
                'classify?version=2016-05-17&url=' + @imgurl

    require 'net/http'
    require 'uri'
    require 'json'

    uri = URI.parse('https://gateway.watsonplatform.net/visual-recognition/'\
                    'api/v3/classify?version=2016-05-17&url=' + @imgurl)
    request = Net::HTTP::Post.new(uri)
    request.basic_auth('apikey', ENV['WATSON_APIKEY'])
    request.content_type = 'application/json'
    request.body = JSON.dump({
  ...
  def classifier
    # IBM Watson Natural Language Classifier allowing us to create custom
    # voice text models
    ...
    uri = URI.parse('https://gateway.watsonplatform.net/natural-'\
  ...
  def nlu
    ...
    # @features = "concepts,categories,emotion,entities,keywords,metadata,"\
    #             "relations,semantic_roles,sentiment"
    ...
    uri = URI.parse('https://gateway.watsonplatform.net/natural-'\
                    'language-understanding/api/v1/analyze?version='\
                    '2018-03-16&features=' + @features + '&text=' + @text)
    request = Net::HTTP::Post.new(uri)
    request.basic_auth('apikey', ENV['WATSON_APIKEY'])
    request.content_type = 'application/json'
    request.body = JSON.dump({
  ...
  def stt
    ...
    # uri = URI.parse("https://gateway.watsonplatform.net/language-translator/"\
    #                 "api/v3/translate?version=2018-05-01")
    # request = Net::HTTP::Post.new(uri)
    # request.basic_auth("apikey", ENV['WATSON_APIKEY'])
    # request.content_type = "application/json"
    # request.body = JSON.dump({
  ...
  def translate
    ...
    @text = params[:text]
    @text = 'hello world!' if @text.to_s == ''

    @startlang = params[:startlang]
    @startlang = 'en' if @startlang.to_s == ''

    @targetlang = params[:targetlang]
    @targetlang = 'es' if @targetlang.to_s == ''
  ...
  def identifylanguage
    ...
    uri = URI.parse('https://gateway.watsonplatform.net/language-translator/'\
                    'api/v3/identify?version=2018-05-01')
    request = Net::HTTP::Post.new(uri)
    request.basic_auth('apikey', ENV['WATSON_APIKEY'])
  ...
  def translatelanguages
    ...
    uri = URI.parse('https://gateway.watsonplatform.net/language-translator/'\
                    'api/v3/identifiable_languages?version=2018-05-01')
  ...
  def getweatherforecast
    @geocode = if params[:geoid].to_s == '' ||
                  params[:geoid].to_s == 'undefined'
                 '35.613,-77.366'
               else
                 params[:geoid]
               end
    ...
    url = URI('https://api.weather.com/v3/wx/forecast/daily/3day?apiKey=' +
              ENV['WEATHER_APIKEY'] +
              '&language=en-US&units=e%0A&format=json&geocode=' + @geocode)
  ...
  def get15minforecast
    @location = if params[:location].to_s == ''
                  '40.712399/-73.964152'
                else
                  params[:location]
                end
    ...
    url = URI('https://api.weather.com/v1/geocode/' + @location +
              '/forecast/fifteenminute.json?apiKey=' + ENV['WEATHER_APIKEY'] +
              '&language=en-US&units=e')
  ...
  def getlocation
    ...
    url = URI('https://api.weather.com/v3/location/search?apiKey=' +
              ENV['WEATHER_APIKEY'] + '&language=en-US&format=json&query=' +
              @location)
  ...
  def weathernowcast
    ...
    url = URI('https://api.weather.com/v1/geocode/' + @location +
              '/forecast/nowcast.json?apiKey=' + ENV['WEATHER_APIKEY'] +
              '&language=en-US&units=e')
  ...
  def powerdisruption
    ...
    url = URI('https://api.weather.com/v2/indices/powerDisruption/daypart/'\
              '15day?apiKey=' + ENV['WEATHER_APIKEY'] +
              '&language=en-US&format=json&geocode=' + @location)
  ...
  def tropicalforecast
    ...
    url = URI('https://api.weather.com/v2/tropical/projectedpath?apiKey=' +
              ENV['WEATHER_APIKEY'] + '&language=en-US&format=json&units=e&'\
              'nautical=true&source=all&basin=' + @basin)
  ...
  def tropicalcurrent
    ...
    url = URI('https://api.weather.com/v2/tropical/currentposition?apiKey=' +
              ENV['WEATHER_APIKEY'] + '&units=e&language=en-US&format=json&'\
              'nautical=true&source=all&basin=' + @basin)
  ...
  def weatheralmanac
    ...
    url = URI('https://api.weather.com/v3/wx/almanac/daily/5day?apiKey=' +
              ENV['WEATHER_APIKEY'] +
              '&units=e&geocode=40.712399%2C-73.964152&startDay=22&'\
              'startMonth=08&format=json')
  ...
  def currentsondemand
    ...
    url = URI('https://api.weather.com/v3/wx/observations/current?apiKey=' +
              ENV['WEATHER_APIKEY'] +
              '&units=e&geocode=40.712399%2C-73.964152&'\
  ...
  def siteobservations
    ...
    url = URI('https://api.weather.com/v1/geocode/40.712399/-73.964152/'\
              'observations.json?apiKey=' + ENV['WEATHER_APIKEY'] +
              '&language=en-US&units=e')
